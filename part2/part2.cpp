...
int main() {
    ...
    std::thread t1(printThread, 1, 40, 300);
    {
        std::lock_guard<std::mutex> lock(cout_mutex);
    ...
    {
        std::lock_guard<std::mutex> lock(cout_mutex);
        std::cout << "Главный поток: поток t1 отсоединён." << std::endl;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(cout_mutex);
        std::cout << "Главный поток ожидает завершения потока t2..." << std::endl;
    }

    t2.join();
    
    std::cout << "Главный поток: поток t2 завершён" << std::endl;
